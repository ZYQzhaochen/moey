...
<body>
	...
	<div style="background:rgba(255,255,255,0.6); border: 2px dashed #C0C0C0; min-height: 50px; font-size: 16px; line-height: 1.5; margin: 5px; border-radius: 0.8em;padding: 10px;">
		...
		<p>调用本站服务或在本站进行投稿，代表您已仔细阅读并同意<a href="tiaokuan.html" style="color: #FF69B4;text-decoration:none;">使用条款</a>。<br>本接口二次元壁纸资源均来源于网络，如果侵害了您的权益，请点击这里<a href="infringment.php" style="color: #FF69B4;text-decoration:none;" target="_blank">侵权申诉</a>。</p>
		...
		<div style="background-color:white;color: #696969;padding:14px;border-left: 6px solid;border-color:#6495ED;">
			<b>请求地址：</b><?php echo filter_input(INPUT_SERVER, "SERVER_NAME"); ?>/wallpaper<br>
			<b>访问协议：</b>HTTPS<br>
			<b>请求方式：</b>GET<br>
			<b>请求示例：</b>https://<?php echo filter_input(INPUT_SERVER, "SERVER_NAME"); ?>/wallpaper
		</div>
		<h2><font color="#FF69B4">#</font> <font color="#696969">请求参数</font></h2>
		<div style="background-color:white;color: #696969;padding:14px;border-left: 6px solid;border-color:#6495ED;">
			<b>type：</b><i>（选填）</i>返回输出类型，填img(图片)/json(json)/text(链接)，不填默认为img(直接返回图片，推荐使用)<br>
			<b>form：</b><i>（选填）</i>图片类型，填level(横屏图片)/vertical(竖屏图片)，不填默认为level<br>
		</div>
		...
		<div style="background-color:white;padding:14px;border-left: 6px solid;border-color:red;">
			<?php
			$s = file_get_contents("http://".$_SERVER['HTTP_HOST']."/wallpaper/?type=json&form=level");
			echo $s;
			?>
		</div>
		<font color="#696969">上方显示的是type值为json、form值为level时的输出结果</font>
		<center><iframe src="../wallpaper/" style="width:100%;height:180px;background:white"></iframe></center>
		<font color="#696969">上方显示的是type值为img、form值为level时的输出结果，您也可以<a href="../wallpaper/" style="color: #FF69B4;text-decoration:none;" target="_blank">戳这里</a>查看实际效果</font>
	</div>
